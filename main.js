...
function sortPlayerHand() {
    for (let j = 0; j < playerHand.cards.length; j++) {
        for (let i = 0; i < playerHand.cards.length - 1; i++) {
            if (
                CARD_SUITS_MAP[playerHand.cards[i].suit] <
                    CARD_SUITS_MAP[playerHand.cards[i + 1].suit] ||
                (CARD_SUITS_MAP[playerHand.cards[i].suit] ==
                    CARD_SUITS_MAP[playerHand.cards[i + 1].suit] &&
                    CARD_VALUE_MAP[playerHand.cards[i].value] <
                        CARD_VALUE_MAP[playerHand.cards[i + 1].value])
            ) {
